...
def bpm_to_tempo(bpm):
    '''Convert BPM to MIDI tempo in microseconds per beat (quarter note).
    '''
    return int(6e7 / bpm)


def tempo_to_bpm(tempo):
    '''Convert MIDI tempo in microseconds per beat (quarter note) to BPM
    '''
    return int(6e7 / tempo)
